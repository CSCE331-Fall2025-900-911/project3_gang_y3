...
import { loginCredentials, loginGoogle, logout } from "../actions";

interface MenuSelectorClientProps {
    session: any;
    userRole: string;
    isManager: boolean;
    isCashier: boolean;
...
export default function MenuSelectorClient({ session, userRole, isManager, isCashier }: MenuSelectorClientProps) {
    const { t } = useLanguage();
                    ...
                    <div className="flex flex-col items-center text-center space-y-8 animate-in fade-in zoom-in duration-500">
                        <div className="space-y-4">
                            <h1 className="text-5xl font-extrabold tracking-tight text-zinc-900 dark:text-zinc-100 bg-gradient-to-r from-blue-600 to-violet-600 bg-clip-text text-transparent">
                                {t("Welcome to our Boba Shop")}
                            </h1>
                            <p className="text-lg text-zinc-600 dark:text-zinc-400 max-w-md mx-auto">
                                {t("Please sign in or continue as a guest.")}
                            </p>
                        </div>
                        ...
                        <div className="w-full max-w-md space-y-6">
                            ...
                            <div className="relative">
                                ...
                                <div className="relative flex justify-center text-xs uppercase">
                                    <span className="bg-zinc-50 dark:bg-zinc-950 px-2 text-zinc-500">{t("Or")}</span>
                                </div>
                            </div>
                        ...
                        <header className="flex flex-col md:flex-row items-center justify-between gap-6 pb-8 border-b border-zinc-200 dark:border-zinc-800">
                            <div>
                                ...
                                <p className="text-zinc-500 dark:text-zinc-400 mt-1">
                                    {t("Logged in as")} <span className="font-semibold text-zinc-800 dark:text-zinc-200">{session.user?.name || session.user?.email}</span>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                            ...
                            >
                                ...
                                <div className="relative z-10 flex flex-col h-full">
                                    ...
                                    <h3 className="text-xl font-bold text-zinc-900 dark:text-zinc-100 mb-2">{t("Customer Kiosk")}</h3>
                                    <p className="text-zinc-500 dark:text-zinc-400 mb-8 flex-grow">
                                        {t("Launch the self-service ordering interface for customers.")}
                                    </p>
                                    <div className="flex items-center text-blue-600 dark:text-blue-400 font-medium group-hover:translate-x-1 transition-transform">
                                        {t("Launch Kiosk")} <span className="ml-2">→</span>
                                    </div>
                                </div>
                            </Link>
                            ...
                            {isManager && (
                                ...
                                >
                                    ...
                                    <div className="relative z-10 flex flex-col h-full">
                                        ...
                                        <h3 className="text-xl font-bold text-zinc-900 dark:text-zinc-100 mb-2">{t("Manager Portal")}</h3>
                                        <p className="text-zinc-500 dark:text-zinc-400 mb-8 flex-grow">
                                            {t("Manage menu items, prices, inventory, and view reports.")}
                                        </p>
                                        <div className="flex items-center text-teal-600 dark:text-teal-400 font-medium group-hover:translate-x-1 transition-transform">
                                            {t("Enter Portal")} <span className="ml-2">→</span>
                                        </div>
                                    </div>
                                </Link>
                            )}
                            ...
                            {(isManager || isCashier) && (
                                ...
                                >
                                    ...
                                    <div className="relative z-10 flex flex-col h-full">
                                        ...
                                        <h3 className="text-xl font-bold text-zinc-900 dark:text-zinc-100 mb-2">{t("Cashier Station")}</h3>
                                        <p className="text-zinc-500 dark:text-zinc-400 mb-8 flex-grow">
                                            {t("Process orders and handle payments manually at the counter.")}
                                        </p>
                                        <div className="flex items-center text-amber-600 dark:text-amber-400 font-medium group-hover:translate-x-1 transition-transform">
                                            {t("Open Station")} <span className="ml-2">→</span>
                                        </div>
                                    </div>
                                </Link>
                            )}
